...
# Function to categorize sentiment into high, medium, or low risk
def categorize_risk(sentiment_score):
    if sentiment_score > 0.5:  # High positive sentiment
        return 'Low Risk'
    elif sentiment_score > 0:  # Slightly positive
        return 'Medium Risk'
    else:  # Negative sentiment
        return 'High Risk'


# Function to perform sentiment analysis and categorize risk
def perform_sentiment_analysis(text):
    blob = TextBlob(text)
    sentiment_score = blob.sentiment.polarity  # Sentiment score between -1 and 1
    risk_category = categorize_risk(sentiment_score)
    return risk_category, sentiment_score
...
def generate_sentiment_dashboard(spam_comments):
    sentiments = []

    # Insert two fake spam comments directly into the spam_comments list
    fake_comments = [
        "click to claim",
        "double your money"
    ]

    # Combine the original spam_comments with the fake ones
    spam_comments.extend(fake_comments)

    # Assuming perform_sentiment_analysis returns risk category and sentiment score
    def perform_sentiment_analysis(comment):
        # Placeholder: You can replace this with an actual sentiment analysis function
        # Example: It returns "Low Risk" and a sentiment score
        return "Low Risk", 0.75

    # Perform sentiment analysis and classify the risk for each spam comment
    for comment in spam_comments:
        risk_category, sentiment_score = perform_sentiment_analysis(comment)
        sentiments.append({
            "text": comment,
    ...
    # Convert the sentiment data to a JSON-friendly string
    sentiment_html = json.dumps(sentiments)

    # HTML structure for the sentiment dashboard (static part)
    dashboard_html = f"""
        <div id="sentiment-dashboard" style="position: fixed; top: 0; left: 0; width: 100%; background-color: #008CBA; color: white;
            ...
            <strong style="font-weight: 700;">Sentiment Analysis Dashboard</strong>
            <div style="margin-top: 10px;">
                <button id="show-sentiments" style="background-color: #FFEB3B; padding: 8px 12px; color: #00796b; border: none; 
    ...
    # JavaScript for functionality of the dashboard and showing sentiment data in a popup
    script = f"""
        (function() {{
            const sentimentData = {sentiment_html};
            const dashboardHtml = `{dashboard_html}`;

            // Add the sentiment dashboard to the body if it doesn't already exist
            if (!document.getElementById('sentiment-dashboard')) {{
                ...
                // Show/Hide the sentiment details in the popup when the button is clicked
                document.getElementById('show-sentiments').onclick = () => {{
                    const popup = document.getElementById('sentiment-popup');
                    popup.style.display = popup.style.display === 'block' ? 'none' : 'block';
                }};
            }}

            // Create and append the popup if not already present
            if (!document.getElementById('sentiment-popup')) {{
                const popup = document.createElement('div');
                popup.id = 'sentiment-popup';
                popup.style.cssText = `
                    position: fixed; top: 80px; right: 30px; width: 400px; padding: 20px;
                    background-color: #ffffff; border-radius: 8px; box-shadow: 0px 6px 20px rgba(0, 0, 0, 0.2);
            ...
            // Function to update the content of the popup with sentiment details
            function updatePopupContent() {{
                const popup = document.getElementById('sentiment-popup');
                popup.innerHTML = "<h4 style='color: black;'>Sentiment Details</h4>";  // Black title color

                sentimentData.forEach(function(comment) {{
                    popup.innerHTML += `
                        <div style="margin-bottom: 15px;">
                            <p><strong>Text:</strong> ${{comment.text}}</p>
                            <p><strong>Sentiment Category:</strong> 
                                <span style="color: ${{
                                    getRiskColor(comment.risk_category)
                                }};">${{comment.risk_category}}</span>
                            </p>
            ...
            // Function to return color based on sentiment risk category
...
API_KEY = 'f121ac0acfmsh255d19a78d4113dp1d579ajsnd923f55e4fbe'  # Replace with your valid API key
...
def open_browser_and_get_url():
    ...
    try:
        ...
        while True:
            ...
            scripts = generate_sentiment_dashboard(spam_comments)
            driver.execute_script(script)
